...
public class MusicFilter {
    ...
    public boolean checkFilter(WebMusicId webId, MusicData music, PatternType pattern, MusicScore score, MusicScore rivalScore) {
        ...
        if (!ESP && pattern == PatternType.ESP && !(AllowExpertWhenNoChallenge && (CSP && music.Difficulty_CSP == 0)))
            return false;
        if (!CSP && pattern == PatternType.CSP && !(AllowOnlyChallenge && ((ESP && music.Difficulty_ESP == 0) || (DSP && music.Difficulty_DSP == 0) || (BSP && music.Difficulty_BSP == 0) || (bSP && music.Difficulty_bSP == 0))))
            return false;
        if (!BDP && pattern == PatternType.BDP) return false;
        if (!DDP && pattern == PatternType.DDP) return false;
        if (!EDP && pattern == PatternType.EDP && !(AllowExpertWhenNoChallenge && (CDP && music.Difficulty_CDP == 0)))
            return false;
        if (!CDP && pattern == PatternType.CDP && !(AllowOnlyChallenge && ((EDP && music.Difficulty_EDP == 0) || (DDP && music.Difficulty_DDP == 0) || (BDP && music.Difficulty_BDP == 0))))
            return false;
        ...
        ScoreData myscore;
        {
            ...
            if (scoreData.PlayCount < PlayCountMin || PlayCountMax < scoreData.PlayCount)
                return false;
            if (scoreData.ClearCount < ClearCountMin || ClearCountMax < scoreData.ClearCount)
                return false;
        ...
        if (!RankAAArival || !RankAAprival || !RankAArival || !RankAAmrival ||
            ...
            if (!FcMFCrival && scoreData.FullComboType == FullComboType.MerverousFullCombo)
                return false;
            ...
            if (scoreData.MaxCombo < MaxComboMinRival || MaxComboMaxRival < scoreData.MaxCombo)
                return false;
            if (scoreData.PlayCount < PlayCountMinRival || PlayCountMaxRival < scoreData.PlayCount)
                return false;
            if (scoreData.ClearCount < ClearCountMinRival || ClearCountMaxRival < scoreData.ClearCount)
                return false;
            ...
            if (dif < ScoreDifferenceMinusMax || (ScoreDifferenceMinusMin < dif && dif < ScoreDifferencePlusMin) || ScoreDifferencePlusMax < dif)
                return false;
            dif = myscore.MaxCombo - scoreData.MaxCombo;
            if (dif < MaxComboDifferenceMinusMax || (MaxComboDifferenceMinusMin < dif && dif < MaxComboDifferencePlusMin) || MaxComboDifferencePlusMax < dif)
                return false;
            dif = myscore.PlayCount - scoreData.PlayCount;
            if (dif < PlayCountDifferenceMinusMax || (PlayCountDifferenceMinusMin < dif && dif < PlayCountDifferencePlusMin) || PlayCountDifferencePlusMax < dif)
                return false;
            dif = myscore.ClearCount - scoreData.ClearCount;
            if (dif < ClearCountDifferenceMinusMax || (ClearCountDifferenceMinusMin < dif && dif < ClearCountDifferencePlusMin) || ClearCountDifferencePlusMax < dif)
                return false;
        }
